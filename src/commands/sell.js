...
class SellCommand extends Command {
  ...
  async exec(message) {
    ...
    const commandUsage = new Discord.MessageEmbed()
      ...
      .addField("Command Usage", `**\`#sell [symbol] [shares(amount)]\`**`)
      .setColor(0xff0000);
    ...
    const noShares = new Discord.MessageEmbed()
      ...
      .setDescription(
        `*You need to provide a positive integer for the amount of shares you want to sell.*`
      )
      .addField("Command Usage", `**\`#sell [symbol] [shares(amount)]\`**`)
      .setColor(0xff0000);
    const unknownSymbol = new Discord.MessageEmbed()
      ...
      .setDescription(
        `*The provided symbol isn't owned by you or doesn't exist.*`
      )
      .setColor(0xff0000);
    const noDevice = new Discord.MessageEmbed()
      ...
      .setColor(0xff0000);
    const insufficientShares = new Discord.MessageEmbed()
      .setAuthor(
        `The Market Error `,
        this.client.users.cache
          .get("882815730486243359")
      ...
      .setDescription(`*Insufficient shares owned to sell this amount.*`)
      .setColor(0xff0000);
    const success = new Discord.MessageEmbed()
      ...
      .setDescription(
        "The shares have been successfully sold. Find more details regarding this in the fields below."
      )
      .setThumbnail(
        this.client.users.cache
          .get("882815730486243359")
    ...
    const fired = new Discord.MessageEmbed()
      ...
      .setDescription(
        `*Oh no. It was discovered that you were not paying attention to your job. Therefore, you've lost your job and need to wait a 1 week cooldown. :(*`
      )
      .setColor(0xff0000);
    ...
    // check whether provided symbol is owned and update array
    let ownsStock = false;
    let totalShares = shares;
    for (var i = 0; i < stocks.length; i++) {
      if (stocks[i].symbol == symbol) {
        if (stocks[i].shares < shares) {
          return message.channel.send({ embeds: [insufficientShares] });
        }
      ...
      embeds: [
        success.addField(
          ...
          `**Transaction Details:**\n` +
            ...
            `\u3000 total amount received: \`${formatter.format(total)}\`\n`
        ),
      ],
    });
